    ...
    public function store(ImageUploadRequest $request) : RedirectResponse
    {
        ...
        for ($a = 1; $a <= count($asset_tags); $a++) {
            ...
            if (($asset_tags) && (array_key_exists($a, $asset_tags))) {
                $device_type = $request->input('_snipeit_device_type_5');
                if (strtolower($device_type) == 'personal') {
                    $code_tag = 'P';
                } else {
                    $code_tag = 'T';
                }
                ...
                // Cek duplikasi asset_tag sebelum assign
                $generated_tag = 'IT-' . $year . $Source .'-' . $code_tag . '-' . $this->getNoTrans($device_type);
                $tag_exists = Asset::where('asset_tag', $generated_tag)->exists();
                $counter = 0;
                while ($tag_exists && $counter < 1000) { // Hindari infinite loop
                    $generated_tag = 'IT-' . $year . $Source .'-' . $code_tag . '-' . $this->getNoTrans($device_type);
                    $tag_exists = Asset::where('asset_tag', $generated_tag)->exists();
                    $counter++;
                }
                $asset->asset_tag = $generated_tag;
...
public function getNoTrans($device_type){
    ...
    if (!$query) {
        // Jika tidak ada, buat record baru dengan order 1
        DB::table('custom_numbering')->insert([
            'name' => $device_type,
            'order' => 1,
            'year' => date('y')
        ]);
